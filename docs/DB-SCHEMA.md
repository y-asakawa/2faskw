...
### 7.1 GraphicalMatrix sequence保存形式

`sequence` 列のprefixは、2FAS-KWの保存形式とバージョンを表す。
`kw1:`、`aesgcm1:`、`hsp1:` の各フィールドはコロンで区切られ、バイナリ値は
URL-safe Base64（paddingなし）で格納される。保存値をDB移行する場合は、prefixを含む
文字列全体を変更せずに移行する。

| 保存方式 | DB保存形式 | 復号可否 | 説明 |
| --- | --- | --- | --- |
| `keyword` | `kw1:<salt>:<iv>:<ciphertext-and-tag>` | 可 | keywordからレコードごとのAES鍵を導出する。 |
| `aes-gcm` | `aesgcm1:<iv>:<ciphertext-and-tag>` | 可 | AES鍵を直接使用する。 |
| `hash` | `hsp1:<selection-count>:<salt>:<hmac>` | 不可 | pepper付きHMACで選択値を照合する。 |

#### keyword方式: `kw1:`

```text
kw1:<salt>:<iv>:<ciphertext-and-tag>
```

- `salt`: レコードごとに生成するランダム16 bytes値。
- `iv`: AES-GCMで使うレコードごとのランダム12 bytes値。
- `ciphertext-and-tag`: AES-GCMの暗号文と128 bitsの認証タグを連結した値。
- 鍵導出: `keywordFile` または `keyword` の文字列から、PBKDF2-HMAC-SHA256を120,000回実行して256 bitsのAES鍵を導出する。
- 暗号化: AES-GCM。

復号には、保存値全体と暗号化時と同一のkeywordが必要である。saltとIVは保存値に含まれるため、
別ファイルで管理する必要はない。

#### AES-GCM鍵方式: `aesgcm1:`

```text
aesgcm1:<iv>:<ciphertext-and-tag>
```

- `iv`: AES-GCMで使うレコードごとのランダム12 bytes値。
- `ciphertext-and-tag`: AES-GCMの暗号文と128 bitsの認証タグを連結した値。
- AES鍵は16 / 24 / 32 bytesである。base64形式で保存した32 bytes鍵を推奨する。

復号には、保存値全体と暗号化時と同一のAES鍵bytesが必要である。鍵ファイルのパスではなく、
鍵ファイルの内容が一致しなければ復号できない。

#### hash方式: `hsp1:`

```text
hsp1:<selection-count>:<salt>:<hmac>
```

- `selection-count`: 登録された画像選択数。
- `salt`: レコードごとに生成するランダム16 bytes値。
- `hmac`: pepperとsaltを使ったHMAC-SHA256値。

`hsp1:` は元の画像ID列を復号できない。認証時は入力された画像ID列から同じHMACを計算し、
保存値と一致するかだけを判定する。

### 7.2 GraphicalMatrixSequenceToolによる確認

`GraphicalMatrixSequenceTool` は、保存済みの `sequence` 値の形式確認や、復号可能な値の
表示に使う管理者向けCLIである。IdPの通常認証では使用しない。ツールは
`graphicalmatrix.properties` とsecret fileを読み込むため、保存形式に対応する設定とsecretを
利用できるIdPサーバ上で実行する。

基本構文:

```text
GraphicalMatrixSequenceTool mode IDP_HOME
GraphicalMatrixSequenceTool storage IDP_HOME storedSequence
GraphicalMatrixSequenceTool display IDP_HOME storedSequence
```

例として、`kw1:` または `aesgcm1:` の保存値を表示する。`STORED` には、DBから取得した
prefixを含む保存値全体を設定する。DB接続情報を扱う取得手順はローカル運用手順に分離する。

```bash
STORED='kw1:<salt>:<iv>:<ciphertext-and-tag>'

CP="$(find /opt/shibboleth-idp/edit-webapp/WEB-INF/lib \
  -maxdepth 1 -type f -name '*.jar' -print | sort | paste -sd ':' -)"

sudo -u jetty java -cp "$CP" \
  io.github.yasakawa.faskw.GraphicalMatrixSequenceTool \
  storage /opt/shibboleth-idp "$STORED"

sudo -u jetty java -cp "$CP" \
  io.github.yasakawa.faskw.GraphicalMatrixSequenceTool \
  display /opt/shibboleth-idp "$STORED"
```

`storage` は保存形式名を出力する。`display` は `keyword` または `aes-gcm` の値を画像ID列として
表示する。`hsp1:` は復号できないため、`display` の対象にしない。復号時は、DB値のprefixと
`graphicalmatrix.sequence.storage`、対応するkeywordまたはAES鍵が一致している必要がある。
