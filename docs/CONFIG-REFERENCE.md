...
| `graphicalmatrix.sequence.storage` | enum | `plaintext` | sequence保存方式。 | `plaintext`, `keyword`, `aes-gcm`, `hash`。本番は`hash`推奨。 |
...
| `graphicalmatrix.totp.seed.storage` | enum | `auto` | TOTP seed保存方式。 | `auto`, `plaintext`, `keyword`, `aes-gcm`。`hash`は不可。 |
...
| `graphicalmatrix.api.allowedCidrs` | CIDR list | `127.0.0.1/32,192.168.0.0/24` | API接続許可CIDR。 | Firewall/LB側でも制限する。 |
