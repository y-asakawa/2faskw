...
| `graphicalmatrix.lockout.lockSeconds` | integer seconds | `900` | 通常ロック時間。 | `1`から`2592000`。既定では5回目から9回目に適用。 |
| `graphicalmatrix.lockout.maxLockFailureCount` | integer | `10` | 最大ロックを開始する保存済み失敗回数。 | `failureLimit`より大きく`1000`以下。 |
| `graphicalmatrix.lockout.maxLockSeconds` | integer seconds | `2592000` | 最大ロック時間。 | `lockSeconds`以上、最大30日。`0`による永久ロックは使用できない。 |
| `graphicalmatrix.savedata` | enum | `db` | GraphicalMatrix/TOTP/MFA方式選択の保存先。 | `db` または `ldap`。WebAuthn StorageServiceの保存先はこの設定では変更しない。 |
...
`graphicalmatrix.savedata=ldap` の場合に、GraphicalMatrix / TOTP / MFA方式選択をLDAPユーザー属性へ保存するための設定。
WebAuthn credentialのLDAP StorageService化は別機能であり、このファイルのフェーズ1設定だけでは切り替わらない。

| Property | Type | Default / Example | Description | Notes |
| --- | --- | --- | --- | --- |
| `graphicalmatrix.ldap.url` | LDAP URL | `ldaps://127.0.0.1:686` | LDAP接続URL。 | 本番はLDAPSを推奨。 |
| `graphicalmatrix.ldap.baseDN` | DN | `OU=people,DC=example,DC=jp` | ユーザー検索起点DN。 | 対象ユーザーが一意に検索できる範囲にする。 |
| `graphicalmatrix.ldap.userFilter` | LDAP filter | `(cn={user})` | ユーザー検索filter。 | `{user}` は認証済みユーザーIDで置換される。 |
| `graphicalmatrix.ldap.subtreeSearch` | boolean | `true` | subtree検索を使うか。 | false時はone-level検索。 |
| `graphicalmatrix.ldap.bindDN` | DN | `CN=graphicalmatrix-writer,...` | LDAP属性読み書き用service account。 | 必要最小限の属性だけ読み書き可能にする。 |
| `graphicalmatrix.ldap.bindCredentialFile` | path | `/opt/shibboleth-idp/credentials/graphicalmatrix-ldap-bind.secret` | bind credentialファイル。 | 配布ZIPへ同梱しない。権限は`0640`以下。 |
| `graphicalmatrix.ldap.connectTimeout` | duration | `PT3S` | LDAP接続タイムアウト。 | 長すぎると認証応答に影響する。 |
| `graphicalmatrix.ldap.responseTimeout` | duration | `PT3S` | LDAP応答タイムアウト。 | LDAP障害時の待ち時間に影響する。 |
| `graphicalmatrix.ldap.attr.sequence` | LDAP attr | `ldap_sequence` | 現在のGraphicalMatrix sequence保存属性。 | `graphicalmatrix.sequence.storage` でencode済みの値を保存する。 |
| `graphicalmatrix.ldap.attr.initial_sequence` | LDAP attr | `ldap_initial_sequence` | 初期sequence保存属性。 | フェーズ1 runtimeでは主に参照外。管理/移行用。 |
| `graphicalmatrix.ldap.attr.status` | LDAP attr | `ldap_status` | ユーザー状態保存属性。 | `ACTIVE` / `DISABLED`。 |
| `graphicalmatrix.ldap.attr.failed_count` | LDAP attr | `ldap_failed_count` | GraphicalMatrix失敗回数。 | 数値文字列で保存する。 |
| `graphicalmatrix.ldap.attr.locked_until` | LDAP attr | `ldap_locked_until` | ロック解除時刻。 | Unix epoch milliseconds。 |
| `graphicalmatrix.ldap.attr.mfa_method` | LDAP attr | `ldap_mfa_method` | 利用MFA方式。 | `GraphicalMatrix`, `TOTP`, `WebAuthn`。 |
| `graphicalmatrix.ldap.attr.totp_seed` | LDAP attr | `ldap_totp_seed` | TOTP seed保存属性。 | `totpaesgcm1:` または `totpkw1:` 形式を推奨。 |
| `graphicalmatrix.ldap.attr.totp_status` | LDAP attr | `ldap_totp_status` | TOTP登録状態。 | `UNREGISTERED`, `PENDING`, `ACTIVE`。 |
| `graphicalmatrix.ldap.attr.totp_registered_at` | LDAP attr | `ldap_totp_registered_at` | TOTP登録完了時刻。 | Unix epoch milliseconds。 |
| `graphicalmatrix.ldap.attr.last_success_at` | LDAP attr | `ldap_last_success_at` | 最終成功時刻。 | Unix epoch milliseconds。 |
| `graphicalmatrix.ldap.attr.force_sequence_change` | LDAP attr | `ldap_force_sequence_change` | sequence変更強制フラグ。 | `0` または `1`。 |
| `graphicalmatrix.ldap.attr.state_version` | LDAP attr | `ldap_state_version` | 楽観ロック用世代番号。 | 変更完了時の古い画面からの上書きを防ぐ。 |
| `graphicalmatrix.ldap.attr.created_at` | LDAP attr | `ldap_created_at` | 作成時刻。 | 移行/管理用途。 |
| `graphicalmatrix.ldap.attr.updated_at` | LDAP attr | `ldap_updated_at` | 更新時刻。 | Unix epoch milliseconds。 |
...
| `idp.authn.webauthn.StorageService` | bean id | `shibboleth.StorageService` | credential保存先StorageService。 | DB/JDBC保存を推奨。LDAP保存を選択する場合は `GraphicalMatrixLDAPStorageService` を指定する。 |
...
WebAuthn credentialをLDAPへ保存する場合のGraphicalMatrix LDAP StorageService設定。
DB/JDBC StorageServiceが推奨構成であり、この設定はLDAP保存を選択した場合のみ使う。

| Property | Type | Default / Example | Description | Notes |
| --- | --- | --- | --- | --- |
| `graphicalmatrix.webauthn.ldap.url` | LDAP URL | fallback to `graphicalmatrix.ldap.url` | WebAuthn StorageService用LDAP接続URL。 | 本番はLDAPS推奨。 |
| `graphicalmatrix.webauthn.ldap.bindDN` | DN | fallback to `graphicalmatrix.ldap.bindDN` | WebAuthn StorageService用service account。 | credential属性だけに権限を絞る。 |
| `graphicalmatrix.webauthn.ldap.bindCredentialFile` | path | fallback to `graphicalmatrix.ldap.bindCredentialFile` | bind credentialファイル。 | 配布ZIPへ同梱しない。 |
| `graphicalmatrix.webauthn.ldap.value.storage` | enum | `plaintext` | LDAP属性へ保存するWebAuthn credential JSONの保存形式。 | 現段階は`plaintext`のみ実装。LDAP ACLで保護する。 |
| `graphicalmatrix.webauthn.ldap.layout` | enum | `subtree` | LDAP保存レイアウト。 | `subtree`推奨。`user-entry`も選択可。 |
| `graphicalmatrix.webauthn.ldap.baseDN` | DN | `OU=WebAuthnStorage,...` | `subtree`方式の保存先DN。 | 1 StorageService record = 1 LDAP entry。 |
| `graphicalmatrix.webauthn.ldap.recordRdnAttr` | LDAP attr | `cn` | `subtree`方式のRDN属性。 | 値は`context + id`のSHA-256由来。 |
| `graphicalmatrix.webauthn.ldap.objectClasses` | list | `top,organizationalRole,extensibleObject` | `subtree`方式で作成するobjectClass。 | 利用LDAPのschemaに合わせる。 |
| `graphicalmatrix.webauthn.ldap.userBaseDN` | DN | fallback to `graphicalmatrix.ldap.baseDN` | `user-entry`方式のユーザー検索起点DN。 | `user-entry`方式で使用。 |
| `graphicalmatrix.webauthn.ldap.userFilter` | LDAP filter | `(cn={id})` | `user-entry`方式のユーザー検索filter。 | `{id}` / `{user}` がStorageService idで置換される。 |
| `graphicalmatrix.webauthn.ldap.userSubtreeSearch` | boolean | `true` | `user-entry`方式でsubtree検索を使うか。 | false時はone-level検索。 |
| `graphicalmatrix.webauthn.ldap.attr.context` | LDAP attr | `gmStorageContext` | StorageService context属性。 | `context + id` の複合キー表現に必要。 |
| `graphicalmatrix.webauthn.ldap.attr.id` | LDAP attr | `gmStorageId` | StorageService id属性。 | 多くの場合ユーザーID相当。 |
| `graphicalmatrix.webauthn.ldap.attr.expires` | LDAP attr | `gmStorageExpires` | 有効期限属性。 | Unix epoch milliseconds。 |
| `graphicalmatrix.webauthn.ldap.attr.value` | LDAP attr | `gmStorageValue` | credential JSON保存属性。 | サイズ上限、index有無、ACLをLDAP側で確認する。 |
| `graphicalmatrix.webauthn.ldap.attr.version` | LDAP attr | `gmStorageVersion` | StorageService version属性。 | 楽観ロック用。 |

`subtree`方式はStorageServiceの `context + id` 複合キーと複数recordに対応しやすいため推奨。
`user-entry`方式はユーザー属性に直接保存できる一方、1ユーザー1record前提であり、
複数contextや複数credential recordが必要な場合は避ける。
