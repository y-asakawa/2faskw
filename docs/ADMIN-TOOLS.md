...
この章では、Admin Tools配布物の中身と、実行環境に必要な前提を整理する。

### 2.1 対応Linux環境

Admin Tools本体はJava 21、bash、JDBCで動作するため、Rocky Linux専用ではない。
ただし、この文書のインストール例、systemd path/service例、firewalld例は
Rocky/RHEL互換環境を前提にしている。
Debian、Ubuntu、その他のLinuxでは、パッケージ名、サービス定義、Firewall設定、
PostgreSQL client toolsの配置を環境に合わせて読み替える必要がある。
...
この章では、Admin Toolsを安全にDBへ接続するための設定と、
IdP側設定との同期が必要な項目を整理する。
...
この章では、基本的なCLI実行方法、代表的な管理操作、dry-run / `--apply` の扱いを整理する。
...
この章では、本番運用時に必要な起動時チェック、監査ログ、配布時の禁止事項を整理する。
...
この章では、外部アカウント管理システムからCSVを受け取り、
Admin Toolsで安全に反映するための設計と実装済み機能を整理する。
