# GraphicalMatrix Admin Tools

## 概要

GraphicalMatrix Admin Tools は、IdPプラグイン本体をインストールせずに、
...
IdPサーバには `graphicalmatrix-idp-plugin-1.0.1.zip` を導入する。
DBサーバまたは管理端末には `graphicalmatrix-admin-tools-1.0.1.zip` を導入する。
...
sudo bash /tmp/graphicalmatrix-admin-extract/graphicalmatrix-admin-tools-1.0.1/bin/graphicalmatrix-admin-install.sh \
  --prefix /opt/graphicalmatrix-admin \
  --apply
