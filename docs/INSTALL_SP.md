...
- SP側ApacheのHTTP/80リダイレクトは、確認時点では
  `https://192.168.0.61/`へ向いている。HTTPSのSP動作には影響しないが、
...
```bash
ssh user@192.168.0.61 'hostnamectl; hostname -f'
ssh user@192.168.0.61 'sudo httpd -t'
ssh user@192.168.0.61 'systemctl is-active httpd php-fpm'
ssh user@192.168.0.61 'systemctl is-enabled httpd php-fpm'
ssh user@192.168.0.61 'sudo firewall-cmd --list-services'
```
...
```bash
ssh user@192.168.0.60 'hostnamectl; hostname -f'
ssh user@192.168.0.60 'systemctl is-active jetty-idp.service'
...
/home/user/install-sp-06-canonical-ip.log
  baseurlpath/entityIDを一時的に https://192.168.0.61/ 系へ変更。
