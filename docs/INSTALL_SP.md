...
```bash
ssh user@192.0.2.61 'hostnamectl; hostname -f'
ssh user@192.0.2.61 'sudo httpd -t'
ssh user@192.0.2.61 'systemctl is-active httpd php-fpm'
ssh user@192.0.2.61 'systemctl is-enabled httpd php-fpm'
ssh user@192.0.2.61 'sudo firewall-cmd --list-services'
```
...
```bash
curl --cacert SP_test/<timestamp>2faskwsp/example-ca.crt \
...
```bash
ssh user@192.0.2.60 'hostnamectl; hostname -f'
ssh user@192.0.2.60 'systemctl is-active jetty-idp.service'
ssh user@192.0.2.60 \
...
SP_test/<timestamp>2faskwsp/example-ca.crt
```
...
sudo install -o root -g root -m 0644 /home/user/example-ca.crt \
  /etc/pki/tls/certs/example-ca.crt
sudo install -o root -g root -m 0600 /home/user/2faskwsp.key \
  /etc/pki/tls/private/2faskwsp.key
sudo sh -c 'cat /etc/pki/tls/certs/2faskwsp.cert.pem /etc/pki/tls/certs/example-ca.crt > /etc/pki/tls/certs/2faskwsp.fullchain.pem'
...
```bash
openssl verify -CAfile /etc/pki/tls/certs/example-ca.crt \
  /etc/pki/tls/certs/2faskwsp.cert.pem
