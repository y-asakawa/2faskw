...
- `storagerecords`はWebAuthn credential登録後に作成される。検証時点では1件保存済み。
...
PoCではStorageServiceのDB永続化を確認後、JDBCAcceleratorを有効化した。
...
PoC用GraphicalMatrix Plugin配布物を`2faskw`へインストールした。
リモートの実ログと現行設定を読み取り専用で再確認し、本節へ反映した。
...
```text
/opt/backups/graphicalmatrix-plugin-<timestamp>
```

`/opt/backups/graphicalmatrix-plugin-<timestamp>`は、存在しない`modules`ディレクトリを
...
上記の`0 users`は、導入直後ログである。
確認時点では、`graphicalmatrix-db.sh list`で4ユーザーが登録済みである。
...
```text
/opt/backups/graphicalmatrix-plugin-hikaricp-<timestamp>
/opt/backups/graphicalmatrix-plugin-overlay-bakfiles-<timestamp>
```

インストールログ:

```text
/home/user/install-logs/23-graphicalmatrix-hikaricp-1.0.1-install-<timestamp>.log
/home/user/install-logs/24-graphicalmatrix-hikaricp-webxml-refresh-<timestamp>.log
/home/user/install-logs/25-graphicalmatrix-hikaricp-build-restart-<timestamp>.log
/home/user/install-logs/26-graphicalmatrix-hikaricp-build-restart-<timestamp>.log
/home/user/install-logs/27-graphicalmatrix-hikaricp-build-restart-<timestamp>.log
/home/user/install-logs/28-graphicalmatrix-hikaricp-build-restart-<timestamp>.log
```
...
sudo mkdir -p /opt/backups/graphicalmatrix-plugin-hikaricp-<timestamp>
sudo cp -a /opt/shibboleth-idp/edit-webapp/WEB-INF/lib \
  /opt/backups/graphicalmatrix-plugin-hikaricp-<timestamp>/lib
sudo cp -a /opt/shibboleth-idp/edit-webapp/WEB-INF/web.xml \
  /opt/backups/graphicalmatrix-plugin-hikaricp-<timestamp>/web.xml
sudo cp -a /opt/shibboleth-idp/conf/graphicalmatrix \
  /opt/backups/graphicalmatrix-plugin-hikaricp-<timestamp>/conf-graphicalmatrix
...
```bash
sudo mkdir -p /opt/backups/graphicalmatrix-plugin-overlay-bakfiles-<timestamp>
sudo find /opt/shibboleth-idp/edit-webapp -type f -name '*.bak.*' \
  -print -exec mv -t /opt/backups/graphicalmatrix-plugin-overlay-bakfiles-<timestamp> {} +
```
