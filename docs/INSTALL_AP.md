...
| Shibboleth IdP | 5.2.2 | `https://example.jp/idp/`で起動済み |
...
インストール作業中のみ、`operator`ユーザーでパスワードレスsudoを使用した。
作業完了後は、不要であれば以下を削除して元に戻す。

```bash
sudo rm -f /etc/sudoers.d/99-operator-nopasswd
sudo visudo -c
```
...
```properties
idp.entityID=https://example.jp/idp/shibboleth
idp.scope=example-u.ac.jp
...
```text
/home/operator/install-logs/01-packages.log
/home/operator/install-logs/02-postgresql-init.log
/home/operator/install-logs/03-prerequisites-layout.log
/home/operator/install-logs/04-jetty-idp-install.log
/home/operator/install-logs/05-idp-install-retry.log
/home/operator/install-logs/06-jetty-service.log
/home/operator/install-logs/07-idp-jetty-modules.log
/home/operator/install-logs/08-final-validation.log
/home/operator/install-logs/09-https-config.log
/home/operator/install-logs/09-https-config-continue.log
/home/operator/install-logs/10-https-validation.log
/home/operator/install-logs/11-ldap-sp-config.log
/home/operator/install-logs/12-ldap-idp-validation.log
/home/operator/install-logs/13-sp-config-removal.log
/home/operator/install-logs/14-ldap-only-validation.log
/home/operator/install-logs/15-totp-webauthn-plugin-config.log
/home/operator/install-logs/16-totp-webauthn-validation.log
```

本書もサーバ上へ配置した。

```text
/home/operator/INSTALL_AP.md
```
...
FQDN: example.jp
IP: 192.0.2.60
HTTPS URL: https://example.jp/idp/
```

証明書:

```text
Subject: C=JP, ST=Nagano, L=Nagano, O=Shinshu University, OU=IIC, CN=example.jp
Issuer: CN=iic_example.jp
SAN: DNS:example.jp, IP:192.0.2.60
...
元の秘密鍵を配置した一時ディレクトリ`/home/operator/https-stage`は、PKCS#12作成後に削除した。

サーバ自身がIIC CAを信頼できるよう、以下にもCA証明書を配置した。

```text
/etc/pki/ca-trust/source/anchors/iic_example.jp.pem
```
...
```bash
sudo hostnamectl set-hostname example.jp
```
...
```text
https://example.jp/idp/status: HTTP 200
```
...
この証明書は公開認証局ではなく`iic_example.jp`によって発行されている。
...
DATE_REDACTEDに、既存PoCサーバー`192.0.2.174`のLDAP Password認証設定を
`2faskw`へ移植した。
...
idp.authn.LDAP.ldapURL = ldap://160.252.245.38:389
...
`192.0.2.174`から移植した。値は作業ログおよび本書には記録していない。
...
`2faskw`へ移植した資格情報は、`192.0.2.174`上の値と文字数・SHA-256が一致している。
...
`192.0.2.174`のSimpleSAMLphp設定も、`2faskw`登録前の状態へ復元した。

取り消し前バックアップ:

```text
2faskw: /opt/backups/pre-sp-removal-20260604161643
192.0.2.174: /opt/backups/pre-2faskw-sp-removal-20260604161643
```
...
LDAP URL: ldap://160.252.245.38:389
...
```properties
idp.authn.webauthn.relyingPartyId = example.jp
...
```text
https://example.jp/idp/profile/admin/webauthn-registration
```
...
```text
/home/operator/install-logs/23-jdbc-storage-plugin.log
/home/operator/install-logs/24-webauthn-storage-schema.log
/home/operator/install-logs/25-webauthn-jdbc-storage-config.log
/home/operator/install-logs/26-webauthn-jdbc-build-restart.log
```
...
`/idp/status`はサーバ自身の送信元IPが`192.0.2.60`として扱われ、
...
DATE_REDACTED 10:09頃、`test-user001@example-u.ac.jp`でWebAuthn credential登録と
...
StorePublicKeyCredential: Added public key credential registration for user 'test-user001@example-u.ac.jp'
ValidateWebAuthnAssertion: WebAuthn authentication succeeded for 'test-user001@example-u.ac.jp'
Shibboleth-Audit.SSO: ... https://example.jp/simplesaml/sp ... Success
...
id: test-user001@example-u.ac.jp
version: 1
value_length: 699
```
...
```text
/home/operator/install-logs/27-webauthn-jdbc-accelerator.log
```
...
DATE_REDACTED 10:15:03 ValidateWebAuthnAssertion: WebAuthn authentication succeeded for 'test-user001@example-u.ac.jp'
DATE_REDACTED 10:15:03 Shibboleth-Audit.SSO: ... https://example.jp/simplesaml/sp ... Success
...
```text
webauthn-list:
  既存の test-user001@example-u.ac.jp credentialを表示できることを確認。
...
- `test-user001`と`test-user001@example-u.ac.jp`は別ユーザーとして扱う。
...
```text
/home/operator/install-media/graphicalmatrix-idp-plugin-0.1.0-SNAPSHOT.zip
/opt/src/graphicalmatrix-idp-plugin-0.1.0-SNAPSHOT
...
```bash
sha256sum /home/operator/install-media/graphicalmatrix-idp-plugin-0.1.0-SNAPSHOT.zip
sudo find /opt/src/graphicalmatrix-idp-plugin-0.1.0-SNAPSHOT -maxdepth 2 \
...
```bash
cd /opt/src/graphicalmatrix-idp-plugin-0.1.0-SNAPSHOT

sudo ./bin/graphicalmatrix-plugin-config.sh \
  --idp-home /opt/shibboleth-idp \
  --package-dir /opt/src/graphicalmatrix-idp-plugin-0.1.0-SNAPSHOT \
  --apply
...
/opt/shibboleth-idp/edit-webapp/WEB-INF/lib/graphicalmatrix-idp-plugin-0.1.0-SNAPSHOT.jar
/opt/shibboleth-idp/edit-webapp/WEB-INF/lib/postgresql-42.7.11.jar
...
sudo ls -l /opt/shibboleth-idp/edit-webapp/WEB-INF/lib/graphicalmatrix-idp-plugin-0.1.0-SNAPSHOT.jar
...
```text
Plugin: io.github.yasakawa.faskw.idp.plugin.authn.graphicalmatrix Current Version: 0.1.0
...
curl -k -o /dev/null -w 'idp=%{http_code}\n' \
  https://example.jp/idp/
curl -k -o /dev/null -w 'change=%{http_code}\n' \
  https://example.jp/idp/graphicalmatrix/change
curl -k -o /dev/null -w 'css=%{http_code}\n' \
  https://example.jp/idp/graphicalmatrix/assets/graphicalmatrix.css
curl -k -o /dev/null -w 'api=%{http_code}\n' \
  https://example.jp/idp/graphicalmatrix-admin/api/v1/health
```
...
```text
/home/operator/install-logs/17-graphicalmatrix-plugin-config.log
/home/operator/install-logs/18-graphicalmatrix-postgresql.log
/home/operator/install-logs/19-graphicalmatrix-build-restart.log
/home/operator/install-logs/20-graphicalmatrix-validation.log
/home/operator/install-logs/21-graphicalmatrix-api-disabled-validation.log
/home/operator/install-logs/22-graphicalmatrix-final-validation.log
```

確認コマンド:

```bash
ls -l /home/operator/install-logs/*graphicalmatrix* \
  /home/operator/install-logs/1[7-9]-* \
  /home/operator/install-logs/2[0-2]-*
```
...
22-graphicalmatrix-final-validation.log
  ...
  API healthがHTTP 404。
  Plugin: io.github.yasakawa.faskw.idp.plugin.authn.graphicalmatrix 0.1.0。
...
SimpleSAMLphp テストSP（`example.jp`）をIdPに登録し、
SSO/SLO動作を確認した。

### 16-1. /etc/hosts への追加

IdPサーバはDNSで`example.jp`を解決できないため、
...
<md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata"
  ...
  <md:SPSSODescriptor
    ...
    <md:SingleLogoutService
        ...
        Location="https://example.jp/simplesaml/module.php/saml/sp/singleLogoutService/2faskwsp"/>

    <md:AssertionConsumerService
        Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST"
        Location="https://example.jp/simplesaml/module.php/saml/sp/assertionConsumerService/2faskwsp"
        index="1"/>
  </md:SPSSODescriptor>
</md:EntityDescriptor>
```
...
DATE_REDACTEDに、IdP/APサーバ `192.0.2.60` のSSH接続元を
...
openssl s_client -connect example.jp:443 -servername example.jp </dev/null
```
...
DATE_REDACTEDに、IdP/APサーバ `192.0.2.60`
（`example.jp`）へ GraphicalMatrix Plugin `1.0.1` を適用した。
...
```bash
scp graphicalmatrix-plugin-local-dist/graphicalmatrix-idp-plugin-1.0.1.zip \
  operator@192.0.2.60:/tmp/graphicalmatrix-idp-plugin-1.0.1.zip
```

サーバ上の配置:

```text
/home/operator/install-media/graphicalmatrix-idp-plugin-1.0.1.zip
/opt/src/graphicalmatrix-idp-plugin-1.0.1
```
...
```text
/home/operator/install-logs/23-graphicalmatrix-hikaricp-1.0.1-install-20260610115951.log
/home/operator/install-logs/24-graphicalmatrix-hikaricp-webxml-refresh-20260610120004.log
/home/operator/install-logs/25-graphicalmatrix-hikaricp-build-restart-20260610120024.log
/home/operator/install-logs/26-graphicalmatrix-hikaricp-build-restart-20260610120048.log
/home/operator/install-logs/27-graphicalmatrix-hikaricp-build-restart-20260610120115.log
/home/operator/install-logs/28-graphicalmatrix-hikaricp-build-restart-20260610120138.log
```

実行した主なコマンド:

```bash
sudo mkdir -p /home/operator/install-media /opt/src /opt/backups
sudo install -m 0644 /tmp/graphicalmatrix-idp-plugin-1.0.1.zip \
  /home/operator/install-media/graphicalmatrix-idp-plugin-1.0.1.zip
sudo rm -rf /opt/src/graphicalmatrix-idp-plugin-1.0.1
sudo unzip -q /home/operator/install-media/graphicalmatrix-idp-plugin-1.0.1.zip -d /opt/src
...
sudo /opt/src/graphicalmatrix-idp-plugin-1.0.1/bin/graphicalmatrix-plugin-config.sh \
  --idp-home /opt/shibboleth-idp \
  --package-dir /opt/src/graphicalmatrix-idp-plugin-1.0.1 \
  --apply

sudo find /opt/shibboleth-idp/edit-webapp/WEB-INF/lib \
  -maxdepth 1 -type f -name 'graphicalmatrix-idp-plugin-*.jar' \
  ! -name 'graphicalmatrix-idp-plugin-1.0.1.jar' -print -delete

sudo /opt/src/graphicalmatrix-idp-plugin-1.0.1/bin/graphicalmatrix-plugin-webxml.sh \
...
sudo unzip -l /opt/shibboleth-idp/war/idp.war | \
  grep -E 'WEB-INF/lib/(graphicalmatrix-idp-plugin|HikariCP|postgresql|core-)'
...
WEB-INF/lib/graphicalmatrix-idp-plugin-1.0.1.jar
...
```bash
curl -k --resolve example.jp:443:127.0.0.1 \
...
```text
DB VIP 192.0.2.64 経由で4ユーザーを参照できた。
```
