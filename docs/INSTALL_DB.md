...
# SQLを抜けて
sudo systemctl status postgresql-18 --no-pager
sudo -u postgres psql -d graphicalmatrix -c '\conninfo'
```
...
role="$(PGPASSWORD="$PG_MONITOR_PASSWORD" psql \
  -h 127.0.0.1 \
...
```bash
sudo -u postgres psql -c 'SELECT pg_promote();' postgres
```

確認:

```bash
sudo -u postgres psql -Atqc 'SELECT pg_is_in_recovery();' postgres
...
```bash
sudo -u postgres psql -d graphicalmatrix
```
...
DB1/DB2のSSH接続元を `192.0.2.0/24` のみに制限した。
...
`sudo vi /var/lib/pgsql/18/data/postgresql.conf` へ追記する。
...
`pg_hba.conf` は上から順に評価され、最初に一致した行が使われる。
そのため、`hostssl` の行はファイル末尾へ追記するのではなく、既存の
`host all all 127.0.0.1/32` や広い許可行より上に配置する。

PostgreSQL 18の初期ファイルにある説明コメントは残してよいが、実際に有効な
認証行は重複させない。すでにファイル先頭に実設定行を追加している場合は、
その先頭ブロックを以下の方針で置き換える。
...
# Local maintenance through Unix domain socket
local      all          all                                     peer

# GraphicalMatrix via local HAProxy backend over SSL
hostssl    graphicalmatrix  graphicalmatrix_app         127.0.0.1/32     scram-sha-256

# HA monitor from local HAProxy/Keepalived scripts
host       postgres     graphicalmatrix_ha_monitor  127.0.0.1/32     scram-sha-256

# Direct IdP/Admin connections over SSL, only if direct DB access is allowed
...
HAProxyをDBサーバ上で動かし、VIP `192.0.2.64:5432` を
`127.0.0.1:5432` へ転送している構成では、PostgreSQLから見る接続元は
IdPのIPではなく `127.0.0.1` になる。そのため、VIP経由のIdP接続を許可するには
`hostssl graphicalmatrix graphicalmatrix_app 127.0.0.1/32 ...` が必要になる。

注意:

- 既存の広い `host ... 0.0.0.0/0` がある場合は削除または狭める
- `host all all 127.0.0.1/32` のような広いローカルTCP許可が残っていると、
  非SSL接続時に `hostssl` 行がスキップされた後、その `host` 行に一致してしまう。
  SSLを強制する場合は削除するか、必要なrole/databaseだけの `hostssl` へ置き換える
...
```text
db-vip:  DNS:db.example.com,  IP Address:192.0.2.64
db-node1: DNS:db1.example.com, IP Address:192.0.2.62
db-node2: DNS:db2.example.com, IP Address:192.0.2.63
```
