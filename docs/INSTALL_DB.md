...
DATE_REDACTEDに、GraphicalMatrix MFA / TOTP / WebAuthn credential保存用PostgreSQLを
DB 2台構成で構築した。
...
```text
/opt/shibboleth-idp/conf/graphicalmatrix/db.properties.local-postgresql.bak.20260609095807
```
...
DATE_REDACTEDに、こちらもDB VIPへ切り替えた。
...
```text
/opt/shibboleth-idp/conf/global.xml.webauthn-local-postgresql.bak.20260609100905
```
...
ログ行の形式を揃えるため、DATE_REDACTEDにDB1/DB2へ以下の運用ログ設定を追加した。

`/var/lib/pgsql/18/data/postgresql.conf`:

```conf
# GraphicalMatrix DB operational logging - DATE_REDACTED
logging_collector = on
log_destination = 'stderr'
log_directory = 'log'
log_filename = 'postgresql-%a.log'
...
```text
DB1: /var/lib/pgsql/18/data/postgresql.conf.logging.bak.20260609104648
DB2: /var/lib/pgsql/18/data/postgresql.conf.logging.bak.20260609104648
```
...
```conf
# GraphicalMatrix DB requested logging prefix - DATE_REDACTED
log_connections = on
log_disconnections = on
log_line_prefix = '[%t]%u %d %p[%l]'
```
...
```text
DB1: /var/lib/pgsql/18/data/postgresql.conf.logprefix.bak.20260609112000
DB2: /var/lib/pgsql/18/data/postgresql.conf.logprefix.bak.20260609112000
```
...
```text
DATE_REDACTED 10:47:01.235 JST [21970] user=graphicalmatrix_app db=graphicalmatrix app=psql client=127.0.0.1 LOG:  接続認証完了
DATE_REDACTED 10:47:01.236 JST [21970] user=graphicalmatrix_app db=graphicalmatrix app=psql client=127.0.0.1 LOG:  接続を切断
```
...
DATE_REDACTED時点では、DB1/DB2ともPostgreSQL SSLは無効。
...
## 19. DATE_REDACTED PostgreSQL TLS実適用記録
